...
(select a.member_id,datediff(min(b.start_dt),a.submit_dt) as days_becoming_goldOgreen,b.tier_id from u_analysis_dw.siebel_member a 
join u_analysis_temp.siebel_s_loy_mem_tier b on a.member_id=b.member_id where tier_id in('1-2RU7','1-2RU6') group by a.member_id,submit_dt,tier_id)
,temp_days_ex_gold as
(select a.member_id,
datediff(date_format(current_Date(),'yyyy-MM-dd hh:mm:ss'),max(b.end_dt)) as last_downgrade_from_gold 
from u_analysis_dw.siebel_member a 
join u_analysis_temp.siebel_s_loy_mem_tier b on a.member_id=b.member_id where a.level='Green' and b.tier_id='1-2RU7' group by a.member_id)
,temp_days_ex_green as
(select a.member_id,
datediff(date_format(current_Date(),'yyyy-MM-dd hh:mm:ss'),max(b.end_dt)) as last_downgrade_from_green 
from u_analysis_dw.siebel_member a 
join u_analysis_temp.siebel_s_loy_mem_tier b on a.member_id=b.member_id where a.level='Welcome' and b.tier_id='1-2RU6' group by a.member_id)
