...
--core
with temp_vchr as (SELECT ROW_ID,PART_NUM,MEMBER_ID,USED_DT,CONSUMED_TXN_ID FROM u_analysis_dw.siebel_s_loy_mem_vchr_used where pdate between date_add(current_date,-365) and date_add(current_date,-1) and used_dt between date_add(current_date,-365) and date_add(current_date,-1) and status_cd='Used' and prod_id in ('BFP_00000000001','BFP_00000000002','1-1ZBAAE8T','1-1ZBAAE93','1-1ZBAAE9D'))
,temp_redeem as (
SELECT t1.*,
...
INSERT OVERWRITE TABLE u_analysis_app.member_campaign_perference_core_benefits partition(pdate='${td}',type='P12M')
